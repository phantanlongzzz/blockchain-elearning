    ...
    <div className="flex items-center justify-between font-mono text-xs py-0.5 group/hash">
      <span className="truncate select-all" title={hash}>
        {prefixHighlight && hash.startsWith('0'.repeat(prefixHighlight)) ? (
          ...
          <span className={isError ? 'text-rose-400 font-semibold' : 'text-slate-300'}>
            {formatted}
          </span>
        )}
      </span>
...
export const BlockchainVisualizer: React.FC = () => {
  ...
  return (
    <section id="blockchain" className="py-6 sm:py-8 max-w-7xl mx-auto font-sans scroll-mt-20 text-slate-100">
      {/* 1. Header Bar */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-slate-800 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 self-start lg:self-auto">
          ...
          <div className="flex items-center bg-slate-900 p-1 rounded-lg border border-slate-800 font-sans text-xs">
            <button
              type="button"
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-rose-950/60 hover:bg-rose-900 text-rose-300 border border-rose-800/80 font-sans font-medium text-xs transition-all cursor-pointer flex items-center gap-1.5"
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-slate-100 border border-slate-800 font-sans font-medium text-xs flex items-center gap-1.5 transition-all cursor-pointer"
          >
      ...
      {guideMode === 'guided' && (
        <div
          ...
          className="mb-6 p-4 rounded-xl bg-[#0c101c] border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 font-sans text-xs"
        >
          ...
          <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
            {guideStepIndex > 0 && (
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-slate-200 border border-slate-800 text-xs font-medium transition-all cursor-pointer font-sans"
              >
                {isVi ? '← Lùi lại' : '← Back'}
              </button>
            )}
            ...
            {guideStepIndex < 2 ? (
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold text-xs transition-all cursor-pointer font-sans"
              >
                {isVi ? 'Bước tiếp →' : 'Next →'}
              </button>
            ) : (
              <button
                type="button"
                onClick={resetChain}
                className="px-3.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium text-xs transition-all cursor-pointer font-sans"
              >
      ...
      {!isChainValid && (
        <div className="mb-6 p-4 rounded-xl bg-rose-950/30 border border-rose-500/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-sans animate-in fade-in duration-150">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-rose-900/60 hover:bg-rose-900 text-rose-200 border border-rose-500/40 text-xs font-medium cursor-pointer transition-colors"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 text-xs font-medium cursor-pointer transition-colors"
            >
      ...
      {minedFeedback && (
        <div className="mb-6 p-3 rounded-xl bg-emerald-950/30 border border-emerald-500/50 flex items-center justify-between text-xs font-sans text-emerald-300 animate-in fade-in duration-150">
          <div className="flex items-center gap-2">
      ...
      {isAuditMatrixExpanded && (
        <div className="mb-6 p-4 rounded-xl bg-[#0c101c] border border-slate-800 space-y-3 font-sans animate-in fade-in duration-150">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
            ...
            <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-800 flex items-center justify-between">
              ...
              <span
                className={`font-semibold text-xs ${
                  isAllLinksValid ? 'text-emerald-400' : 'text-rose-400'
                }`}
              >
                {isAllLinksValid
                  ? isVi ? 'ĐẠT' : strings.blockchain.pass
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3 px-1">
          <div className="flex items-center gap-2">
            <h3 className="text-xs font-sans font-semibold text-slate-300 tracking-wider">
          ...
          {isChainValid && (
            <button
              type="button"
              onClick={() => setIsAuditMatrixExpanded(!isAuditMatrixExpanded)}
              className="text-xs text-slate-400 hover:text-slate-200 transition-colors cursor-pointer font-sans"
            >
                ...
                <div className="min-w-[260px] sm:min-w-[280px] max-w-[320px] flex-1 flex flex-col shrink-0">
                  <div
                    className={`p-4 rounded-xl border transition-all duration-200 flex flex-col justify-between flex-1 shadow-sm relative ${
                      isInvalid
                        ? isCascading
                          ? 'bg-rose-950/80 border-rose-400 ring-2 ring-rose-500/50'
                          : 'bg-[#180a0e] border-rose-500/70'
                  ...
                  >
                    <div>
                      ...
                      <div className="flex items-center justify-between pb-2 mb-3 border-b border-slate-800">
                        <div className="flex items-center gap-1.5 font-mono">
                          <span className="text-sm font-bold text-slate-100">#{block.index}</span>
                          {isGenesis && (
                      ...
                      <div className="mb-2.5">
                        ...
                        <div
                          className={`w-full bg-[#080c16] border rounded-lg p-2 text-xs font-mono min-h-[50px] leading-relaxed break-words whitespace-pre-wrap transition-colors ${
                            isInvalid
                      ...
                      {isHashAltered && block.originalHash && (
                        <div className="mb-3 p-2.5 rounded-lg bg-[#080c16] border border-rose-800/60 text-[11px] font-mono space-y-1.5 animate-in fade-in duration-100">
                          ...
                          <div className="text-[10px] text-slate-400 font-sans pt-1 border-t border-slate-800 leading-tight">
                            {isVi
                      ...
                      <div className="mb-3">
                        <button
                          ...
                          className="w-full py-1 text-slate-400 hover:text-slate-200 text-[11px] font-sans flex items-center justify-between transition-colors cursor-pointer"
                        >
                        ...
                        {isExpanded && (
                          <div className="mt-2 pt-2 border-t border-slate-800/80 space-y-2.5 text-[11px] font-sans animate-in fade-in duration-100">
                            <div>
                              ...
                              <textarea
                                ...
                                className="w-full bg-[#080c16] border border-slate-700 rounded p-2 text-xs font-mono text-slate-200 focus:outline-none focus:border-emerald-500/60 resize-none"
                              />
                            </div>
                            <div>
                              <span className="text-slate-500 text-[10px] uppercase font-mono block">Timestamp</span>
                              <span className="text-slate-300 font-mono text-xs">{block.timestamp}</span>
                            </div>
                            <div>
                              ...
                              <input
                                ...
                                className="w-full bg-[#080c16] border border-slate-800 rounded px-2 py-1 text-xs text-slate-200 font-mono focus:outline-none focus:border-emerald-500/60"
                              />
                            </div>
                          </div>
                        )}
                    ...
                    <div className="pt-2 border-t border-slate-800/60">
                      <button
                        ...
                        className={`w-full py-1.5 px-2 rounded-lg font-sans text-xs font-medium transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
                          isInvalid
                    ...
                    <div className="relative flex flex-col items-center justify-center w-full">
                      ...
                      <svg className="w-full h-8 overflow-visible" viewBox="0 0 44 24">
                        <defs>
                          ...
                          >
                            <path
                              ...
                              fill={isLinkToNextValid ? '#10b981' : '#f43f5e'}
                            />
                          </marker>
                        </defs>
                        ...
                        <line
                          ...
                          x2="36"
                          y2="12"
                          stroke={isLinkToNextValid ? '#10b981' : '#f43f5e'}
                          strokeWidth="2"
                          strokeDasharray={isLinkToNextValid ? 'none' : '3,3'}
                          strokeOpacity={isLinkToNextValid ? 0.75 : 0.65}
      ...
      <div id="blockchain-deep-dive" className="rounded-xl bg-[#0c101c] border border-slate-800 overflow-hidden font-sans">
        <button
          type="button"
        ...
        >
          ...
          <div className="text-slate-400 flex items-center gap-1.5 text-xs font-sans">
        ...
        {isDeepDiveOpen && (
          <div className="p-4 sm:p-6 border-t border-slate-800 space-y-6 bg-[#090d16] animate-in fade-in duration-150">
            {/* Core Explanations Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans">
              <div className="p-4 rounded-lg bg-[#0c101c] border border-slate-800 space-y-2">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                <div className="p-3.5 rounded-lg bg-[#0c101c] border border-slate-800 space-y-1.5">
            ...
            <div className="pt-2 border-t border-slate-800">
              <span className="text-slate-400 text-[11px] font-sans font-semibold block mb-2">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
                {[
                  ...
                  <div key={i} className="group relative inline-block">
                    <span className="px-2.5 py-1 bg-slate-900 border border-slate-800 text-slate-300 rounded-lg cursor-help hover:border-slate-700 hover:text-slate-100 transition-colors font-medium">
                      {item.term}
                    </span>
                    <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 p-3 bg-[#0c101c] border border-slate-700 rounded-lg text-xs font-sans text-slate-200 shadow-xl opacity-0 group-hover:opacity-100 transition-all duration-150 z-50">
                      <div className="font-semibold text-slate-100 mb-1 border-b border-slate-800 pb-1">
                        {item.term}
                      </div>
      ...
      {isAddBlockModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm font-sans">
          <div className="relative w-full max-w-lg bg-[#0c101c] border border-slate-800 rounded-xl p-6 shadow-2xl text-xs space-y-4 text-slate-200 font-sans animate-in fade-in duration-150">
            ...
            <form onSubmit={handleAddBlock} className="space-y-4">
              <div>
                <label className="text-slate-400 text-[10px] block mb-1 font-sans font-medium">
                ...
                <textarea
                  ...
                  className="w-full bg-[#080c16] border border-slate-800 rounded-lg p-3 text-xs text-slate-200 focus:outline-none focus:border-emerald-500/60 font-mono resize-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 font-sans">
                <button
                  ...
                  className="px-4 py-2 bg-slate-900 text-slate-400 hover:text-slate-200 border border-slate-800 rounded-lg cursor-pointer text-xs font-medium"
                >
                  {isVi ? 'Hủy' : 'Cancel'}
                </button>
                <button
