...
export const StepCompletionFeedback: React.FC<StepCompletionFeedbackProps> = ({
  ...
  if (inline) {
    return (
      <div
        role="status"
        aria-live="polite"
        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-950/70 border border-emerald-500/40 text-emerald-300 text-xs font-sans shadow-sm animate-in fade-in duration-200 ${className}`}
      >
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`p-3 sm:p-3.5 rounded-xl bg-gradient-to-r from-emerald-950/50 via-[#11161d] to-amber-950/30 border border-emerald-500/30 shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-sans animate-in fade-in slide-in-from-top-1 duration-200 ${className}`}
    >
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="min-w-0">
          <div className="text-[11px] font-mono text-emerald-400 font-semibold uppercase tracking-wide flex items-center gap-1">
