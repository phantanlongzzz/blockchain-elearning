...
export const HashVisualizer: React.FC<HashVisualizerProps> = ({ hex, binary, bytes }) => {
  ...
  return (
    <div
      ...
      className="rounded-xl bg-[#0C0F14] border border-[#1C2430] p-5 sm:p-7 shadow-sm font-sans"
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#1C2430] pb-4 mb-5">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-sans text-[#00C98D] font-bold uppercase tracking-wider">
        ...
        {/* View mode toggle */}
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-lg bg-[#090A0F] p-1 border border-[#1C2430]">
            <button
              onClick={() => setViewMode('words')}
              className={`px-3 py-1 text-xs font-sans rounded-md transition-all flex items-center gap-1.5 cursor-pointer ${
                viewMode === 'words'
                  ? 'bg-[#00C98D]/15 text-[#00C98D] border border-[#00C98D]/40 shadow-sm font-medium'
          ...
          <button
            ...
            onClick={copyHash}
            className="px-3 py-1.5 rounded-lg bg-[#0F131A] hover:bg-[#11161E] border border-[#1C2430] text-xs font-sans text-[#00C98D] hover:border-[#00C98D]/40 transition-all flex items-center gap-1.5 cursor-pointer"
      ...
      <div className="space-y-4">
            ...
            {words.map((word, idx) => (
              <div
                ...
                className={`group p-4 sm:p-5 rounded-lg border transition-all duration-200 cursor-pointer ${
                  activeWordIndex === idx
                    ? 'bg-[#11161E] border-[#00C98D]/60 shadow-sm ring-1 ring-[#00C98D]/40'
                    : 'bg-[#090A0F] border-[#1C2430] hover:border-[#2A3649] hover:bg-[#0F131A]'
                }`}
              >
                <div className="flex items-center justify-between text-[11px] font-mono tracking-[0.05em] tabular-nums mb-2">
                  <span className="font-bold text-[#00C98D]">Word H{idx}</span>
                  <span className="text-[#717B8C] font-medium">Bits {idx * 32}–{idx * 32 + 31}</span>
                </div>
                <div className="font-mono text-lg sm:text-xl font-bold tracking-[0.05em] tabular-nums text-[#F2F4F7] group-hover:text-[#00C98D] transition-colors break-all select-all my-1.5">
                  {word}
                </div>
                <div className="mt-2.5 text-[11px] font-mono tracking-[0.05em] tabular-nums flex justify-between items-center border-t border-[#1C2430] pt-2">
                  <span className="text-[#717B8C] font-medium">Bytes {idx * 4}–{idx * 4 + 3}</span>
                  <span className="text-[#00C98D] font-semibold">0x{word}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {formattedBytes.map((byte, idx) => (
              <div
                ...
                className={`p-2.5 rounded-md border text-center transition-all cursor-pointer ${
                  activeByteIndex === idx
                    ? 'bg-[#00C98D]/15 border-[#00C98D] text-[#00C98D] shadow-sm ring-1 ring-[#00C98D]/40'
                    : 'bg-[#090A0F] border-[#1C2430] text-[#A5AFBF] hover:border-[#2A3649]'
                }`}
              >
                <span className="text-[10px] font-mono tracking-[0.05em] tabular-nums text-[#717B8C] font-medium block">B{idx}</span>
                <span className="font-mono text-sm font-bold tracking-[0.05em] tabular-nums text-[#F2F4F7] mt-0.5 block">{byte}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {viewMode === 'binary' && (
          <div>
            <div className="grid grid-cols-16 sm:grid-cols-32 gap-1 p-3.5 bg-[#090A0F] rounded-lg border border-[#1C2430]">
                ...
                return (
                  <div
                    ...
                    className={`aspect-square rounded-sm flex items-center justify-center font-mono tracking-[0.05em] tabular-nums text-[9px] sm:text-[10px] transition-all cursor-crosshair ${
                      isHovered
                        ? 'bg-[#00C98D] text-[#090A0F] font-extrabold scale-125 z-10 shadow-sm'
                        : isOne
                          ? 'bg-[#00C98D]/20 text-[#00C98D] border border-[#00C98D]/40 hover:border-[#00C98D] font-semibold'
                          : 'bg-[#0F131A] border border-[#1C2430] text-[#717B8C] hover:text-[#A5AFBF]'
                    }`}
            ...
            <div className="flex items-center justify-between text-[11px] font-mono tracking-[0.05em] tabular-nums text-[#717B8C] font-medium mt-2 px-1">
              <span>Bit 0 (MSB)</span>
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-sm bg-[#00C98D]/40 border border-[#00C98D] inline-block" /> 1-bit
                </span>
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-sm bg-[#0F131A] border border-[#1C2430] inline-block" /> 0-bit
                </span>
              </div>
        ...
        <div className="mt-4 p-4 rounded-lg bg-[#090A0F] border border-[#1C2430] flex flex-wrap items-center justify-between gap-4 text-xs font-mono tracking-[0.05em] tabular-nums font-sans">
          <div className="flex items-center gap-2 font-sans">
            <Info className="w-4 h-4 text-[#00C98D] flex-shrink-0" />
            <span className="text-[#F2F4F7] font-medium">
              {isVi ? 'Đang kiểm tra Byte vị trí:' : 'Inspecting Byte Index:'}
            </span>
            <span className="px-2.5 py-0.5 rounded-md bg-[#0F131A] border border-[#1C2430] text-[#00C98D] font-bold font-mono tracking-[0.05em] tabular-nums">
              {isVi ? `Byte #${activeByteIndex} (trên 32)` : `Byte #${activeByteIndex} (of 32)`}
            </span>
          </div>

          <div className="flex flex-wrap items-center gap-4 text-[#F2F4F7] font-mono">
            <div>
              <span className="text-[#717B8C] font-medium mr-1.5 font-sans">Hex:</span>
              <span className="text-[#00C98D] font-bold tracking-[0.05em] tabular-nums">0x{activeByteHex}</span>
            </div>
            <div>
              <span className="text-[#717B8C] font-medium mr-1.5 font-sans">{isVi ? 'Nhị phân:' : 'Binary:'}</span>
              <span className="text-[#F2F4F7] font-bold tracking-[0.05em] tabular-nums">{activeByteBin}</span>
            </div>
            <div>
              <span className="text-[#717B8C] font-medium mr-1.5 font-sans">{isVi ? 'Thập phân:' : 'Decimal:'}</span>
              <span className="text-[#F2F4F7] font-bold tracking-[0.05em] tabular-nums">{activeByteDec}</span>
            </div>
            <div>
              <span className="text-[#717B8C] font-medium mr-1.5 font-sans">ASCII:</span>
              <span className="text-[#00C98D] font-bold tracking-[0.05em] tabular-nums">{activeByteAscii}</span>
            </div>
            <div>
              <span className="text-[#717B8C] font-medium mr-1.5 font-sans">{isVi ? 'Từ Word:' : 'Word:'}</span>
